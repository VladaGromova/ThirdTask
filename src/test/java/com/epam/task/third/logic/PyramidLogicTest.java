...
public class PyramidLogicTest {
    ...
    private final static Pyramid FALSE_PYRAMID = new Pyramid(Arrays.asList(0, 0, 0,5, 0, 0, 3, 2, 0, 4, 0, 0));
    ...
    public void testCountPyramidVolumeShouldCountVolumeWhenInputDataIsCorrect(){
        ...
        double expected = 3;
        //when
        double actual = pyramidLogic.countPyramidVolume(TEST_PYRAMID);
        //then
    ...
    public void testCountPyramidVolumeShouldReturnZeroWhenPointsOnOnePlane(){
        ...
        double expected = 0;
        //when
        double actual = pyramidLogic.countPyramidVolume(FALSE_PYRAMID);
        //then
        Assert.assertEquals(expected, actual, DELTA);
    }
    ...
    public void testCountPyramidSurfaceAreaShouldCountPyramidSurfaceAreaWhenInputDataIsCorrect(){
        ...
        //when
        double actual = pyramidLogic.countPyramidSurfaceArea(TEST_PYRAMID);
        //then
        Assert.assertEquals(expected, actual, DELTA);
    }
    ...
    public void testIsPyramidShouldReturnTrueWhenPointsFormPyramid(){
        Assert.assertTrue(pyramidLogic.isPyramid(firstPoint, secondPoint, thirdPoint, fourthPoint));
    }

    @Test
    public void testIsPyramidShouldReturnFalseWhenThereAreEqualPoints(){
        boolean actual = pyramidLogic.isPyramid(firstPoint, firstPoint, thirdPoint, fourthPoint);
        Assert.assertFalse(actual);
    }

    @Test
    public void testIsBaseOnCoordinatePlaneShouldReturnTrueWhenItIs(){
        boolean actual = pyramidLogic.isBaseOnCoordinatePlane(PYRAMID_ON_PLANE);
        Assert.assertTrue(actual);
    }

    @Test
    public void testIsBaseOnCoordinatePlaneShouldReturnTrueWhenItIsNot(){
        boolean actual = pyramidLogic.isBaseOnCoordinatePlane(TEST_PYRAMID);
        Assert.assertFalse(actual);
    }
}
